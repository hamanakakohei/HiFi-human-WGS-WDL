...
workflow joint {
  ...
  call split_vcf_by_sample_2 as split_glnexus {
    input:
      sample_ids            = sample_ids,
      vcf                   = glnexus.vcf,
...
task split_vcf_by_sample_2 {
  ...
  Int threads   = 8
  Int mem_gb    = 8
  Int memory    = ceil(mem_gb / threads) # edited by hamanaka
  ...
  command <<<
    ...
    bcftools --version
    bcftools +split -W=tbi -Oz -o ./ ~{vcf}

    for sample_id in ~{sep=" " sample_ids}; do
      # Extract sample, optionally excluding uncalled genotypes
      mv ${sample_id}.vcf.gz     ${sample_id}.~{vcf_basename}.vcf.gz
      mv ${sample_id}.vcf.gz.tbi ${sample_id}.~{vcf_basename}.vcf.gz.tbi
  ...
  runtime {
    docker: "~{runtime_attributes.container_registry}/pb_wdl_base:sha2564b889a1f21a6a7fecf18820613cf610103966a93218de772caba126ab70a8e87"
    cpu: threads
    #memory: mem_gb + " GiB"
